"""Analyze risk usage under selected insurance parameter settings."""
...
def build_model(HARD_ALPHA, beta_k, HARD_DELTA, enforce_equity: bool):
    ...
    for f in F:
        for p in P[f]:
            ...
            if USE_INSURANCE:
                prem = P_fp[(f, p)] * A0 * N[(f, p)]
                unins = LAMBDA * P_fp[(f, p)] * C_firm[(f, p)] * N[(f, p)]
            else:
                prem = 0
                unins = 0
...
def objective_breakdown(m):
    ...
    prem_total = sum(P_fp[(f, p)] * A0 * N[(f, p)].X for f in F for p in P[f])
    ship_total = sum(LAMBDA * P_fp[(f, p)] * C_firm[(f, p)] * N[(f, p)].X for f in F for p in P[f])
...
if not USE_FIXED_RISK:
    ...
    HARD_ALPHA = 1450   # paste from baseline
    beta_k = {
        "K1": 482.176435,
        "K2": 20.021425,
        "K3": 189.290908,
