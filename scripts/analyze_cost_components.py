"""Compare operational, premium, and uninsured-loss cost components."""
import itertools
...
def build_model(
    ...
    if use_insurance:
        if A0_val is None or LAMBDA_val is None or C_firm_use is None:
            raise ValueError("When use_insurance=True, must pass A0_val, LAMBDA_val, and C_firm_use.")
    ...
    for f in F:
        for p in P[f]:
            ...
            if use_insurance:
                prem = P_fp_use[(f, p)] * A0_val * N[(f, p)]
                unins = LAMBDA_val * P_fp_use[(f, p)] * C_firm_use[(f, p)] * N[(f, p)]
            else:
                prem = 0.0
                unins = 0.0
...
def objective_breakdown(m, A0_val, LAMBDA_val, P_fp_use, C_firm_use):
    ...
    prem_total = sum(
        P_fp_use[(f, p)] * A0_val * N[(f, p)].X
        for f in F for p in P[f]
    )

    ship_total = sum(
        LAMBDA_val * P_fp_use[(f, p)] * C_firm_use[(f, p)] * N[(f, p)].X
        for f in F for p in P[f]
    )
...
m0 = build_model(HARD_ALPHA=HUGE, beta_k=beta_loose, HARD_DELTA=HUGE, enforce_equity=False, use_insurance=False)
m0.optimize()
...
mA = build_model(HARD_ALPHA=HARD_ALPHA, beta_k=beta_loose, HARD_DELTA=HUGE, enforce_equity=False, use_insurance=False)
mA.optimize()
...
for eta in etas:
    ...
    m1 = build_model(HARD_ALPHA=HARD_ALPHA, beta_k=beta_try, HARD_DELTA=HUGE, enforce_equity=False, use_insurance=False)
    m1.optimize()
...
m1 = build_model(HARD_ALPHA=HARD_ALPHA, beta_k=beta_k, HARD_DELTA=HUGE, enforce_equity=False, use_insurance=False)
m1.optimize()
if m1.status != GRB.OPTIMAL:
...
for s in range(NUM_INSTANCES):
    ...
    # ---- BASELINE FOR THIS SAME INSTANCE: NO INSURANCE
    m_base = build_model(
        HARD_ALPHA=HARD_ALPHA,
        beta_k=beta_k,
        HARD_DELTA=HARD_DELTA,
    ...
    m_base.Params.OutputFlag = 0
    m_base.optimize()

    if m_base.status != GRB.OPTIMAL:
        print(f"Baseline missing for instance {s}")
        continue

    baseline_flow = {
        (f, p): m_base._N[(f, p)].X for f in F for p in P[f]
    }

    total_shipments = sum(D_trips[f] for f in F)

    # ---- RUN ALL INSURANCE CONFIGS
    for A0_val, L_val in itertools.product(A0_levels, LAMBDA_levels):
        ...
        for f in F:
            for p in P[f]:
                base_val = baseline_flow.get((f, p), 0.0)
                curr_val = current_flow.get((f, p), 0.0)
                change += abs(curr_val - base_val)
...
df.to_csv(OUTPUT_DIR / "cost_analysis_rerouting_results.csv", index=False)
...
for s in range(NUM_INSTANCES):
    ...
    for A0_val, L_val in itertools.product(A0_levels, LAMBDA_levels):
        ...
        ops, prem, ship, total = objective_breakdown(
            m,
            A0_val=A0_val,
            LAMBDA_val=L_val,
            P_fp_use=P_fp_use,
        ...
        cost_results.append({
            ...
            "A0": A0_val,
            "lam": L_val,
            "oper_pct": 100 * ops / total,
            "prem_pct": 100 * prem / total,
            "unins_pct": 100 * ship / total
        })
...
cost_df.to_csv(OUTPUT_DIR / "cost_composition_results.csv", index=False)

cost_avg = (
    cost_df.groupby(["A0", "lam"], as_index=False)[["oper_pct", "prem_pct", "unins_pct"]]
           .mean()
)
...
# ===============================
oper = (
    cost_avg.pivot(index="A0", columns="lam", values="oper_pct")
            .reindex(index=A0_levels, columns=LAMBDA_levels)
            .to_numpy()
)

prem = (
    cost_avg.pivot(index="A0", columns="lam", values="prem_pct")
            .reindex(index=A0_levels, columns=LAMBDA_levels)
            .to_numpy()
)

unins = (
    cost_avg.pivot(index="A0", columns="lam", values="unins_pct")
            .reindex(index=A0_levels, columns=LAMBDA_levels)
            .to_numpy()
)
...
# ===============================
A0_labels = ["1M", "7M", "10M"]
lambda_labels = [15, 30, 50]

x = np.arange(len(A0_labels)) *1.5
width = 0.20
...
alpha_vals = [0.9, 0.75, 0.6]
for i, lam in enumerate(lambda_labels):
    offset = (i - 1) * width * 1.2

    plt.bar(x + offset, oper[:, i], width,
            color=colors["oper"],
            label="Operational" if i == 0 else "")

    plt.bar(x + offset, prem[:, i], width,
            bottom=oper[:, i],
            color=colors["prem"],
            label="Premium" if i == 0 else "")

    plt.bar(x + offset, unins[:, i], width,
            bottom=oper[:, i] + prem[:, i],
            color=colors["unins"],
            label="Uninsured" if i == 0 else "")

plt.xticks(x, A0_labels)
plt.xlabel("Insurance Parameter A(0)")
plt.ylabel("Cost Composition (%)")
plt.title("Cost Composition of Objective Function Across Insurance Scenarios")
...
plt.savefig(OUTPUT_DIR / "cost_composition_by_scenario.png", dpi=300, bbox_inches="tight")
plt.close()
from statsmodels.formula.api import ols
