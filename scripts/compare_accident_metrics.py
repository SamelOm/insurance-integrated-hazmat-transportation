"""Compare expected-accident metrics with and without insurance terms."""
...
# MODEL BUILDER
# -------------------------
def build_model(HARD_ALPHA, beta_k, HARD_DELTA, enforce_equity: bool):
    ...
    for f in F:
        for p in P[f]:
            ...
            prem = 10*P_fp[(f, p)] * A0 * N[(f, p)]
            unins = LAMBDA * 10*P_fp[(f, p)] * C_firm[(f, p)] * N[(f, p)]
...
def objective_breakdown(m):
    ...
    prem_total = sum(10*P_fp[(f, p)] * A0 * N[(f, p)].X for f in F for p in P[f])
    ship_total = sum(10*LAMBDA * P_fp[(f, p)] * C_firm[(f, p)] * N[(f, p)].X for f in F for p in P[f])
...
A0_backup = A0
LAMBDA_backup = LAMBDA

A0 = 0
LAMBDA = 0

m = build_model(HARD_ALPHA, beta_k, HARD_DELTA, True)
m.optimize()

baseline_total = sum(m._N[(f,p)].X for f in F for p in P[f])
baseline_acc = sum(P_fp[(f,p)] * m._N[(f,p)].X for f in F for p in P[f])

baseline_ratio = baseline_total / baseline_acc

print("\n[Baseline accidents]")
print("Expected accidents:", baseline_acc)
print("Shipments per accident:", baseline_ratio)
# -------------------------
# FINAL SOLVE
# -------------------------
print("\n=== FINAL SOLVE (alpha+beta+equity) ===")
m = build_model(HARD_ALPHA=HARD_ALPHA, beta_k=beta_k, HARD_DELTA=HARD_DELTA, enforce_equity=True)
m.optimize()
...
ins_total = sum(m._N[(f,p)].X for f in F for p in P[f])
ins_acc = sum(P_fp[(f,p)] * m._N[(f,p)].X for f in F for p in P[f])

ins_ratio = ins_total / ins_acc

print("\n[Insurance accidents]")
print("Expected accidents:", ins_acc)
print("Shipments per accident:", ins_ratio)
print("\n[Comparison]")
print("Change in expected accidents:", (ins_acc - baseline_acc) / baseline_acc)
print("Change in shipments/accident:", (ins_ratio - baseline_ratio) / baseline_ratio)
