"""Measure shipment rerouting relative to a no-insurance baseline."""
import itertools
...
for s in range(NUM_INSTANCES):
    ...
    for A0_val, L_val in itertools.product(A0_levels, LAMBDA_levels):
        ...
        m = build_model(
            ...
            A0_val=A0_val,
            LAMBDA_val=L_val,
            P_fp_use={k: 5 * v for k, v in P_fp_scn.items()},
            C_firm_use=C_firm_scn
        )
...
df.to_csv(OUTPUT_DIR / "rerouting_results.csv", index=False)
...
# ===== AGGREGATE =====
plot_df = (
    df.groupby(["A0", "lam"])["rerouted"]
      .agg(["mean", "std"])
      .reset_index()
)

plot_df["se"] = plot_df["std"] / np.sqrt(NUM_INSTANCES)

A0_order = [1_000_000, 7_000_000, 10_000_000]
lam_order = [15, 30, 50]
colors = ["#4C72B0", "#DD8452", "#55A868"]

pivot_mean = (
    plot_df.pivot(index="A0", columns="lam", values="mean")
           .reindex(index=A0_order, columns=lam_order)
)

pivot_se = (
    plot_df.pivot(index="A0", columns="lam", values="se")
           .reindex(index=A0_order, columns=lam_order)
)

# ===== PLOT =====
x = np.arange(len(A0_order))
width = 0.2

fig, ax = plt.subplots(figsize=(9, 5))

for i, lam in enumerate(lam_order):
    ax.bar(
        x + (i - 1) * width,
        pivot_mean[lam],
        width,
...
ax.set_xlabel(r"Insurance Parameter $A(0)$")
ax.set_ylabel("% Rerouted (vs Baseline)")
ax.set_title("Percentage of Shipments Rerouted Relative to Baseline")

ax.set_xticks(x)
ax.set_xticklabels([f"{a0/1e6:.0f}M" for a0 in A0_order])
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "rerouting_by_insurance_parameters.png", dpi=300, bbox_inches="tight")
plt.close()
from statsmodels.formula.api import ols
...
summary.to_csv(OUTPUT_DIR / "rerouting_summary.csv", index=False)

print("\nRerouting summary by configuration:")
print(summary)

print("\nMean range:")
print(summary["mean"].min(), summary["mean"].max())

print("\nSD range:")
print(summary["std"].min(), summary["std"].max())

print("\n95% CI half-width range:")
print(summary["ci95"].min(), summary["ci95"].max())
