"""Solve the insurance-integrated Erie County hazmat routing model.

The script calibrates system, corridor, and equity limits before solving the
final mixed-integer model. It uses the path-level probabilities supplied in
``data/master_data.xlsx`` and treats corridor 9 as an uncapped "OTHER" group
unless ``CAP_OTHER`` is enabled.
"""
